classdef imtool3DROIaw_ellipse < imtool3DROIaw_rect
    ...
    methods
        %constructor
        function ROI = imtool3DROIaw_ellipse(varargin)
            ...
            ROI@imtool3DROIaw_rect(imageHandle, position, tool, id)
        ...
        function newPosition(ROI,position)
            ...
            %Update fig
            %[meanROI, stdROI] = roiVector(ROI.timeVolume, stats, ROI.tool.handles.Slider.Value);
...
function [sic, stdSic] = roiVector(vol, roi, slice)
    ...
    timePoints = size(vol{1}, 4);

    maskedVol = double(vol{1}(:,:,slice,:)) .* roi.mask;
