...
classdef imtool3Daw < handle
    ...
    methods
        ...
        function tool = imtool3Daw(varargin)  %Constructor
            if ~isdeployed
            ...
            % Parse Inputs
            [I, position, h, range, tools, mask, enableHist, dataHandle] = parseinputs(varargin{:});

            tool.datahandler = dataHandle;
            ...
            fun=@(scr,evnt)multipleScrollWheel(scr,evnt,[tool tool.handles.toolsToSync]);
            ...
            mapNames={'Gray','Parula','Jet','Inverted Jet','HSV','Hot','Cool','Spring','Summer','Autumn','Winter','Bone','Copper','Pink','Lines','colorcube','flag','prism','white'};
...
%             set(tool.handles.Tools.mask2poly ,'Callback',fun)
            ...
            %Create figure for ROI curves

%             figROI = figure;
%             tool.handles.ROIfig.fig = figROI;
%             tool.handles.ROIfig.axes = axes(tool.handles.ROIfig.fig);
%             %hold(tool.handles.ROIfig.axes, 'on');
%             set(tool.handles.ROIfig.fig,'Position',[-800 400 1000 800]);
%             title(tool.handles.ROIfig.axes, 'Signal Intensity Curves');
%             set(tool.handles.ROIfig.axes,'FontSize',20);
%             xlabel(tool.handles.ROIfig.axes, 'Time');
%             ylabel(tool.handles.ROIfig.axes, 'Absolute Signal Intensity');
        ...
        function displaySavedROI(tool, position)
            h = getHandles(tool);
            ROI = imtool3DROIaw_ellipse(h.I, position, tool);
        end

        function setNewColorMap(tool, Value)
            h = tool.getHandles;
            h.Tools.Color.Value = Value;
            hObject = h.Tools.Color;
            eventdata = 'Programmatic change of value';
            changeColormap(hObject,eventdata,tool);
            
        end
            
        function storeToolsToSync(tool, tools)
            tool.handles.toolsToSync = tools;
        end
        
        function setPosition(tool,position)
            set(tool.handles.Panels.Large,'Position',position, tool)
        end
        ...
        function setImage(tool, varargin)
            [I, position, h, range, tools, mask, enablehist, dataHandlee] = parseinputs(varargin{:});
        ...
        function shortcutCallback(tool, evnt)
          ...
          tools = tool.handles.toolsToSync;
          tools = [tool tools];
            ...
            switch evnt.Key
                ...
                case 'leftarrow'
                    if isprop(evnt,'Modifier') && ~isempty(evnt.Modifier) && strcmp(evnt.Modifier,'shift')
                        tools(i).Ntime = max(tools(i).Ntime-10,1);
                    else
                        tools(i).Ntime = max(tools(i).Ntime-1,1);
                    end
                    showSlice(tools(i));
                case 'rightarrow'
                    if isprop(evnt,'Modifier') && ~isempty(evnt.Modifier) && any(strcmp(evnt.Modifier,'shift'))
                        tools(i).Ntime = min(tools(i).Ntime+10,size(tools(i).I{tool.Nvol},4));
                    else
                        tools(i).Ntime = min(tools(i).Ntime+1,size(tools(i).I{tool.Nvol},4));
                    end
                    showSlice(tools(i));
                case 'uparrow'
...
function changeColormap(hObject,eventdata,tool)
    ...
    %Added option of inverting jet
    if n == 4
        jet = colormap(h.Axes,'Jet');
        colormap(h.Axes, flipud(jet));
        if isfield(h,'HistImageAxes')
            colormap(h.HistImageAxes,flipud(jet));
        end
    else
...
function [I, position, h, range, tools, mask, enableHist, dataHandler] = parseinputs(varargin)
            switch length(varargin)
                ...
                case 1  %tool = imtool3d(I)
                    ...
                    range=[]; tools=[]; mask=[]; enableHist=true; dataHandler = [];
                ...
                case 7  %tool = imtool3d(I,position,h,range,tools,mask)
                    I=varargin{1}; position=varargin{2}; h=varargin{3};
                    range=varargin{4}; tools=varargin{5}; mask=varargin{6};
                    enableHist = varargin{7}; dataHandler = [];
                case 8 %tool = imtool3d(I,position,h,range,tools,mask, dataHandler)
                    I=varargin{1}; position=[0 0 1 1]; h=varargin{3};
                    range=varargin{4}; tools=[]; mask=[]; enableHist=true; dataHandler = varargin{8};
            end
...
switch type
    case 'ellipse'
        h = getHandles(tool);
        ROI = imtool3DROIaw_ellipse(h.I,[],tool);
    case 'rectangle'
        h = getHandles(tool);
        ROI = imtool3DROIaw_rect(h.I,[],tool);
    case 'polygon'
        h = getHandles(tool);
        ROI = imtool3DROIaw_poly(h.I,[],tool);
    case 'profile'
        h = getHandles(tool);
        ROI = imtool3DROIaw_line(h.I,[],tool);
    otherwise
end

end

function varargout = imageButtonDownFunction(hObject,eventdata,tool, tools)
switch nargout
    case 0
        ...
        switch get(tool.handles.fig,'SelectionType')
            ...
            case 'extend'  %Zoom
                tools = [tool tool.handles.toolsToSync;];
                ...
                fun=@(src,evnt) adjustZoomMouse(src,evnt,bp,tool.handles.Axes,tools,xlims,ylims,bpA);
                fun2=@(src,evnt) buttonUpFunction(src,evnt,tools(1),WBMF_old,WBUF_old);
                set(tools(1).handles.fig,'WindowButtonMotionFcn',fun,'WindowButtonUpFcn',fun2)
            ...
            case 'alt' %pan
                ...
                fun=@(src,evnt) adjustPanMouse(src,evnt,bp,tool.handles.Axes,xlims,ylims,scale, tools);
                fun2=@(src,evnt) buttonUpFunction(src,evnt,tool,WBMF_old,WBUF_old);
                set(tool.handles.fig,'WindowButtonMotionFcn',fun,'WindowButtonUpFcn',fun2)
        end
    case 2
...
function adjustZoomMouse(src,~,bp,hObject,tools,xlims,ylims,bpA)
...
if resize > 0
    for i = 1:length(tools)
        tools(i).handles.Axes.XLim = xlims;
        tools(i).handles.Axes.YLim = ylims;
    end
...
function adjustPanMouse(src,evnt,bp,hObject,xlims,ylims,scale, tools)
cp = get(0,'PointerLocation');
V = get(hObject,'View');
d = scale*(bp-cp);
if V(1)==-90
...
for i = 1:length(tools)
    tools(i).handles.Axes.XLim = xlims+d(1);
    tools(i).handles.Axes.YLim = ylims-d(2);
end
