classdef imtool3DROIaw_rect < imtool3DROIaw
    ...
    methods
        ...
        %constructor
        function ROI = imtool3DROIaw_rect(varargin)
            ...
            menuLabels = {'Export stats','Fix Aspect Ratio','Hide Text','Delete','poly2mask', 'Save inflow function/ROI', 'Save outflow function/ROI', ...
                'Other ROI'};
            ...
            ROI@imtool3DROIaw(imageHandle,graphicsHandles,menuLabels,menuFunction, tool, id);
...
function contextMenuCallback(source,callbackdata,ROI, tool, varargin)
...
switch get(source,'Label')
    ...
    case 'Save inflow function/ROI'
        stats = getMeasurements(ROI);
        stats.slice = tool.handles.Slider.Value;
        tool.datahandler.saveAIF(stats);
    case 'Save outflow function/ROI'
        stats = getMeasurements(ROI);
        stats.slice = tool.handles.Slider.Value;
        tool.datahandler.saveVIF(stats);
    case 'Other ROI'
        stats = getMeasurements(ROI);
        n = input([ ...
            'Enter ROI number\n', ...
        ...
        stats.slice = tool.handles.Slider.Value;
        tool.datahandler.saveExtraROI(stats, n);
